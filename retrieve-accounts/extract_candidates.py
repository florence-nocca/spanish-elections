...
# This is the updated list (nov 24), replacing the candidates_list_old.xml (nov 18) 
    candidates = codecs.open("datafiles/candidates_database.csv", "w", "utf-8")
    candidates.write(('"%s","%s","%s","%s","%s","%s","%s"\n') % ("name", "sex", "election_type", "district", "party", "initials", "position"))
    page = pq(filename = "candidates_list.xml")
    district = ""
    election_type = ""
    party = ""
    initials = ""
...
def addInformations(name):
    data = codecs.open("datafiles/candidates_database_old.csv","r","utf-8")
    sex = ""
    for line in data:
